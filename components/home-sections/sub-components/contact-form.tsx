...
export const ContactForm = () => {
  ...
  return (
    <div className={cn("h-[25em] w-[25em] bg-white rounded-sm z-10 text-[--text-inverse] flex flex-col justify-between p-4", cascadia.className)}>
          ...
          <AnimatePresence>
            {stage < 3 && stage >= 0 && (
              ...
              >
                ...
                <motion.button
                  onClick={nextStage}
                  className="rounded-md w-fit bg-[--background] text-[--text-primary] p-2 absolute"
                  initial={{ x: 0 }}
                  animate={
                    stage > 0
                      ? {
      ...
      <div className="bg-[--foreground] w-full h-[15%]">
        Hate forms? Connect at <br /> <a href="mailto:nitish@nitish.info">nitish@nitish.info</a>
      </div>
    </div>
  );
};
